<template>
  <div>
    <div class="relative mx-auto max-w-5xl">
      ...
      <img src="https://tft-dna-brand-architects-cc.s3.af-south-1.amazonaws.com/image-gallery/SATA-Assets/Exhibitor+Map.png" alt="Conference Map" class="w-full" />
      <!-- Booth Logos -->
      <div v-for="booth in booths" :key="booth.id" class="absolute hover:scale-110 transition-transform cursor-pointer"
        :style="{
          left: `${booth.position.x}%`,
          top: `${booth.position.y}%`,
          width: `${booth.size || 6}%`
        }" @click="openBoothDetails(booth)">
        <div class="bg-black p-1 rounded h-14">
    ...
    <Dialog v-model:visible="visible" pt:root:class="!bg-transparent !border-0" pt:mask:class="backdrop-blur-sm"
      ...
      <template #container="{ closeCallback }">
        <div class="flex flex-col gap-6 mx-auto px-8 py-8 rounded-2xl w-full max-w-sm" style="background-color: #000">
          <!-- Header -->
...
const booths = ref([
  ...
  },
  {
    ...
    standNumber: '5',
    member: 'Anna Smith',
    logoUrl: 'https://tft-dna-brand-architects-cc.s3.af-south-1.amazonaws.com/image-gallery/Exhibitors/BofiNet-Logo.png',
